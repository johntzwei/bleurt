...
def _make_eager_predict_fn_from_checkpoint(checkpoint):
  ...
  def _predict_fn(input_dict):
    return bleurt_model_ops(
        ...
        group=tf.constant(input_dict["group"], dtype=tf.int64),
        group_mean=tf.constant(input_dict["group_mean"])
        )["predictions"].numpy()
...
class BleurtScorer(object):
  ...
  def score(self, references, candidates, batch_size=None):
    ...
    for i in range(0, len(candidates), batch_size):
      ...
      # add extra parameters "group & group mean"
      group = [0]*batch_size
      group_mean = [0.0]*batch_size
      tf_input = {
          "input_ids": input_ids,
